...
def load_latest_staging_data(_s3_helper, base_path, file_prefix):
    ...
    try:
        ...
        if not files:
            st.warning(
                f"No staging data found with prefix '{file_prefix}'. Please run the Wise pipeline."
            )
            return None
    ...
    chart = (
        ...
        .encode(
            ...
            tooltip=[
                ...
                alt.Tooltip(
                    "closing_balance:Q", title="Closing Balance", format=",.2f"
                ),
            ],
        )
        .interactive()
        .properties(title="Daily Net Change", width=400, height=300)
    )
    return chart
...
def create_balance_chart(df):
    ...
    chart = (
        ...
        .encode(
            x=alt.X("date:T", title="Date"),
            y=alt.Y(
                "closing_balance:Q", title="Balance (£)", scale=alt.Scale(zero=False)
            ),
            tooltip=[
                alt.Tooltip("date:T", title="Date"),
                alt.Tooltip(
        ...
        .interactive()
        .properties(title="Daily Closing Balance", width=400, height=300)
    )
    return chart
...
def create_calendar_heatmap(df):
    ...
    chart = (
        ...
        .properties(title="Cash Flow Calendar Heatmap", width=600, height=200)
    )
    return chart
